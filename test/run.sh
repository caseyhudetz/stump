...
suites="$*"
if [ -z "$suites" ]; then
  suites="chk trail swipe link spot third tidy trim card queue sheet dup keep across four photo pop vec wlog"
...
for s in $suites; do
  ...
  if echo "$out" | grep -q "page errors *: *none" || [ "$s" = wlog ]; then
    printf 'ok    %-8s\n' "$s"
  else
